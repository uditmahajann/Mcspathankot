      ...
      <div className="relative h-[80vh] w-full overflow-hidden">
        {PHOTOS.map((photo, index) => (
          ...
          >
            ...
            />
            {/* Caption overlay */}
            <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/90 via-black/60 to-transparent p-9 text-white">
              <p className="text-2xl font-poppins tracking-wide font-semibold">{photo.caption}</p>
            </div>
          </div>
        ))}
        ...
        <button
          onClick={prev}
          className="absolute left-5 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-3 rounded-full transition cursor-pointer"
        ...
        <button
          onClick={next}
          className="absolute right-5 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-3 rounded-full transition cursor-pointer"
